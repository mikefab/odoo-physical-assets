...
class MaintenanceJobOrder(models.Model):
    ...
    x_studio_category = fields.Char('Category', store=True, copy=True)
    x_studio_currency_id = fields.Many2one('res.currency', store=True, copy=True)
    x_studio_date = fields.Date('Date', store=True, copy=True)
    x_studio_date_start = fields.Datetime('Start Date', store=True, copy=True)
    x_studio_date_stop = fields.Datetime('End Date', store=True, copy=True)
...
class MaintenanceJobOrderAssignment(models.Model):
    ...
    x_studio_3 = fields.Char('Work 5', store=True, copy=True) # S'up?
    ...
    x_studio_company_id = fields.Many2one('res.company', store=True, copy=True, ondelete='Set NULL')
    ...
    x_studio_user_id = fields.Many2one('res.users', string='Responsible', store=True, copy=True, ondelete='Set NULL', domain="[('share', '=', False)]")
    x_studio_value = fields.Float('Value', store=True, copy=True)
    x_studio_many2one_field_BZWhr = Many2one('maintenance.equipment', string='Equipment', store=True, readonly=True, ondelete='Set NULL')
    x_studio_many2one_field_GSELf = Many2one('maintenance.request', string='Description of fault', store=True, readonly=True, ondelete='Set NULL')
    ...
    x_studio_partner_phone = fields.Char('Phone', store=True, copy=True)
    x_studio_priority = fields.Boolean('High Priority')
    x_studio_related_field_gRR5c= fields.Char('New Related Field', readonly=True)
    x_studio_sequence = fields.integer('Sequence', store=True, copy=True)
    x_studio_stage_id = fields.Many2one('x_job_order_assignment_stage', string='Stage', store=True, copy=True, required=True, ondelete='restrict')
...
class MaintenanceEquipment(models.Model):
    ...
    x_studio_year_commissioned = fields.Char(string='Year Commissioned', store=True, copy=True)
    x_studio_estimated_lifespan_years_1 = fields.Selection([('3', '3'), ('5', '5'), ('7', '7'), ('11', '11'), ('13', '13'), ('15', '15')], string='Estimated life span', store=True, copy=True, readonly=False)
    x_studio_acquisition_year = fields.Char(store=True, copy=True)
    x_studio_functional = fields.Selection([('Yes', 'Yes'), ('No', 'No'), ('Under repair', 'Under repair')], store=True, copy= True)
    x_studio_manufacturer = fields.Char(store=True, copy=True, string="Manufacturer")
    x_studio_country_of_origin = fields.Char(store=True, copy=True, string="Country of Origin")
    x_studio_asset_no = fields.Char(store=True, copy=True, string="Asset No")
    x_studio_item_image = fields.Binary(store=True, copy=True)
...
class MaintenanceRequest(models.Model):
    ...
    x_studio_many2one_field_IBYga = fields.Many2one('maintenance.team', store=True, readonly=False, state="manuel")
